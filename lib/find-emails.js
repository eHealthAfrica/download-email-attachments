...
module.exports = function findEmails (args, callback) {
  ...
  var timeout = setTimeout(function () {
    if (state.connection === 'virgin') {
      ...
      console.log('timeout hit unmet case')
      imapDisconnect()
    }

    maybeErrorCallback({abort: true})
  ...
  var maybeErrorCallback = function (args) {
    if ((state.connection === 'closed' && state.queue.idle()) || args.abort) {
      clearTimeout(timeout)

      maybeErrorCallback = function () {
        args.log.error('callback double-called')
      }
      ...
      console.log('callback called on active connection')
    }
  }
  ...
  if (args.debug) {
    imapOptions.debug = function (info) {
      console.log(info)
    }
  }
