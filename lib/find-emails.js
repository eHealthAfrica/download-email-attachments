...
module.exports = function findEmails (args, callback) {
  ...
  imap.on('error', function (err) {
    ...
    if (err.code === 'ECONNRESET' && state.connection === 'closed') {
      ...
      // As in this case we cannot know if the last emails were lost, we need to collect the errors in which case the checkpoint is not done and
      // this batch of emails is reprocessed on the next polling interval
  ...
  function processMails () {
    imap.search([ [ 'SINCE', args.since ] ], function (err, results) {
      ...
      results = filter(results, function (uid) {
      ...
      async.each(results, function (result, resultCallback) {
        ...
        fetcher.on('message', function (msg, seqno) {
          ...
          msg.on('attributes', function (attrs) {
            ...
            state.lastSyncIds.push(attrs.uid)
          })
